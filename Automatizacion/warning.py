# from hhrr_app.automation.connection_db import connection, execute_query
            ...
            if tte[4] == 1:
                ...
                if difference.days >=1:
                    ...
                    if evaluator[0] and evaluator[0] != "NO@NO.COM":
                        enviar_correo(destinatatio=evaluator[0], msg=message)
                    else:
                        enviar_correo(destinatatio=evaluator[1], msg=message)
                ...
                if difference.days >=1:
                    ...
                    if colaborator[0] and colaborator[0] != "NO@NO.COM":
                        enviar_correo(destinatatio=colaborator[0], msg=message)
                    else:
                        enviar_correo(destinatatio=colaborator[1], msg=message)
                ...
                if difference.days >=1:
                    ...
                    if manager[0] and manager[0] != "NO@NO.COM":
                        enviar_correo(destinatatio=manager[0], msg=message)
                    else:
                        enviar_correo(destinatatio=manager[1], msg=message)
            else: return


if __name__ == '__main__':
    scheduler = BackgroundScheduler()
    scheduler.add_job(warning_email, 'cron', hour=6, minute=37, timezone="America/Bogota")
    scheduler.start()
